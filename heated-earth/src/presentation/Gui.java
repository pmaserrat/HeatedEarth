...
class GuiConfig {
	...
	static final int DIM_WIDTH_FRAME				= Math.max(DIM_WIDTH_JPG,DIM_WIDTH_OPTIONS);
	...
	// Time format
	static final DateFormat	DATE_FORMAT				= new SimpleDateFormat("hh:mm a, MMM dd, yyyy");
}
...
//	private JSlider				visDelaySlider = null;
	...
	private EarthPanel earthPanel = new EarthPanel(
			new Dimension(GuiConfig.DIM_WIDTH_JPG,GuiConfig.DIM_HEIGHT_JPG), 
			new Dimension(GuiConfig.DIM_WIDTH_JPG,GuiConfig.DIM_HEIGHT_JPG), 
			new Dimension(GuiConfig.DIM_WIDTH_JPG,GuiConfig.DIM_HEIGHT_JPG)
			);		
	...
	private Gui() {
		...
		// Need to make sure that when the user closes the program that we actually exit
		// the JFrame and go back out to the calling method... this will also help us to
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (command.equals(GuiConfig.ACTION_RUN)) {
			...
			try {
				simTimeCal.setTime(GuiConfig.DATE_FORMAT.parse(GuiConfig.START_TIME));
			...
			// redraw the grid during updates of the grid spacing... it looks cool
...
//		visDelaySlider.setEnabled(bEnable);
	...
	private void createGui() {
		...
		this.getContentPane().setPreferredSize(new Dimension(GuiConfig.DIM_WIDTH_FRAME, GuiConfig.DIM_HEIGHT_FRAME));
		...
		optionsPanel.setBorder( BorderFactory.createMatteBorder(0, 1, 0, 1, Color.blue) );
...
//		earthPanel.setMapOpacity((float)0.5);
	...
	private JPanel createOptionsPanel() {
		...
		panel.setPreferredSize(new Dimension(GuiConfig.DIM_WIDTH_OPTIONS, GuiConfig.DIM_HEIGHT_OPTIONS));
		panel.setLayout(new BorderLayout());
		
		// Add some new dimensions that only have meaning in this context
		int DIM_WIDTH_OPTIONS_LABELS 	= GuiConfig.DIM_WIDTH_OPTIONS*4/7*1/4;
		int DIM_WIDTH_OPTIONS_EDITS		= GuiConfig.DIM_WIDTH_OPTIONS*4/7*3/4;
		...
		// This is a little weird, but we're going to nest our panels.  We've got a main panel
		// for the config options and it contains a panel for the labels on the left and a panel
		// for the actual edit boxes and units on the right.  We do this so that we can align
		// our edit boxes and make things look professional.
		Border simBorder = BorderFactory.createTitledBorder("Configuration Options");
		JPanel configOpts = new JPanel(new BorderLayout());
		JPanel configOptsLabels = new JPanel(new FlowLayout(FlowLayout.LEFT));	// Change the vertical spacing to accommodate for controls
		JPanel configOptsEdits = new JPanel(new FlowLayout(FlowLayout.LEFT)); 
		configOptsLabels.setPreferredSize(new Dimension(DIM_WIDTH_OPTIONS_LABELS,GuiConfig.DIM_HEIGHT_OPTIONS));
		configOptsEdits.setPreferredSize(new Dimension(DIM_WIDTH_OPTIONS_EDITS,GuiConfig.DIM_HEIGHT_OPTIONS));
		configOpts.setBorder(simBorder);
		int EDIT_BOX_WIDTH = 4;		// Make all edit boxes the same width... it looks better
		int LABEL_HEIGHT = 26;
		...
		gridSpacingDegEdit = new PosAngleDegField(EDIT_BOX_WIDTH,GuiConfig.DEFAULT_GRID_SPACING);
		...
		gridSpacingDegSlider = new JSlider(JSlider.HORIZONTAL,1,180,GuiConfig.DEFAULT_GRID_SPACING);
		...
		configOptsLabels.add(tmpLabel); configOptsEdits.add(gridSpacingDegSlider); configOptsEdits.add(gridSpacingDegEdit); configOptsEdits.add(tmpUnits);

		// Simulation Rate
		tmpLabel = new JLabel("Simulation Rate:");
		tmpUnits = new JLabel("min/sim");
		tmpLabel.setPreferredSize(new Dimension(DIM_WIDTH_OPTIONS_LABELS,LABEL_HEIGHT));
		simRateEdit = new PosNumberField(EDIT_BOX_WIDTH,10);	// Default simulation rate is 10 minutes / step
		...
		configOptsLabels.add(tmpLabel); configOptsEdits.add(simRateSlider); configOptsEdits.add(simRateEdit); configOptsEdits.add(tmpUnits);

		// Visualization Rate
		tmpLabel = new JLabel("Visualization Rate:");
		tmpUnits = new JLabel("sim/pres");
		tmpLabel.setPreferredSize(new Dimension(DIM_WIDTH_OPTIONS_LABELS,LABEL_HEIGHT));
		visRateEdit = new PosNumberField(EDIT_BOX_WIDTH,1);	// Default visualization rate is 1 step / presentation
		...
		configOptsLabels.add(tmpLabel); configOptsEdits.add(visRateSlider); configOptsEdits.add(visRateEdit); configOptsEdits.add(tmpUnits);
	
		// Simulation Delay
		tmpLabel = new JLabel("Simulation Delay:");
		tmpUnits = new JLabel("ms/step");
		tmpLabel.setPreferredSize(new Dimension(DIM_WIDTH_OPTIONS_LABELS,LABEL_HEIGHT));
		simDelayEdit = new PosNumberField(EDIT_BOX_WIDTH,GuiConfig.DEFAULT_SIM_DELAY);
		simDelayEdit.setEditable(false);
		simDelaySlider = new JSlider(JSlider.HORIZONTAL,0,2000,GuiConfig.DEFAULT_SIM_DELAY);
		...
		configOptsLabels.add(tmpLabel); configOptsEdits.add(simDelaySlider); configOptsEdits.add(simDelayEdit); configOptsEdits.add(tmpUnits);
		
//		// Visualization Delay
//		tmpLabel = new JLabel("Visualization Delay:");
//		tmpUnits = new JLabel("ms/pres");
//		tmpLabel.setPreferredSize(new Dimension(DIM_WIDTH_OPTIONS_LABELS,LABEL_HEIGHT));
//		visDelayEdit = new PosNumberField(EDIT_BOX_WIDTH,0);	// Default simulation rate is 10 minutes / step
...
//		configOptsLabels.add(tmpLabel); configOptsEdits.add(visDelaySlider); configOptsEdits.add(visDelayEdit); configOptsEdits.add(tmpUnits);
//		
		...
		configOptsLabels.add(tmpLabel); configOptsEdits.add(threadUse_Sim); configOptsEdits.add(threadUse_Pres); configOptsEdits.add(tmpUnits);
		...
		group.add(comConfig_Push); group.add(comConfig_Pull); group.add(comConfig_Buff);
		configOptsLabels.add(tmpLabel); configOptsEdits.add(comConfig_Push); configOptsEdits.add(comConfig_Pull); configOptsEdits.add(comConfig_Buff);
		
		// Now add the options to the panel
		configOpts.add(configOptsLabels,BorderLayout.WEST); configOpts.add(configOptsEdits,BorderLayout.EAST);
 ...
 * 
 * Provides validation on a text field to enforce that values are
 * numbers and that they are positive.
 *
 */
 ...
 * 
 * Provides validation on a text field to enforce that values are
 * angles ([0,360) degrees) and that they are positive.
 *
 */
	...
	static class posAngleDegDocument extends PlainDocument {
		...
		public void insertString(int offs, String str, AttributeSet a) throws BadLocationException {
			...
			try {
				...
				int[] legalVals = {1,2,3,4,5,6,9,10,12,15,18,20,30,36,45,60,90,180};
	    		...
	    		if (num > 0 && num <= 180) {
	    			super.insertString(offs, new String(Integer.valueOf(finalNum).toString()), a);
	    		}
			} catch (NumberFormatException e) {
				// do nothings... we don't care about it
			}
