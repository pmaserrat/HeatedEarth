...
public class Grid implements TemperatureGrid{
        ...
        public int getSpacing() {return gs_deg;}
        
        public Grid(Grid other) {
                this.initialize(other.getSpacing(), 0);
                
                // The temperature is the only thing that differentiates one grid from another
                for (int i = 0; i < this.rows; i++) {
                        for (int j = 0; j < this.cols; j++) {
                                this.myGrid[i][j].setTemperature(other.getCell(i,j).getTemperature());
                        }
                }
...
//                              float lenBot = SimConstant.C * (float)Math.cos((latitude-0.5*gs)*Math.PI/180) * p;
                                float lenBot = (float) (2 * Math.PI * SimConstant.R * Math.cos(Math.toRadians(lat_deg)) * p_deg);
                                
//                              float lenTop = SimConstant.C * (float)Math.cos((lat_deg-0.5*gs + gs)*Math.PI/180) * p;
                                float lenTop = (float) (2 * Math.PI * SimConstant.R * Math.cos(Math.toRadians(lat_deg + gs_deg)) * p_deg);

                                float h = (float) Math.sqrt(Math.pow(lenVer, 2) - 0.25 * Math.pow(lenBot - lenTop, 2));
...
//              }
        ...
        public float getCellHeight(int x, int y) {
                ...
                float height = (float)(Math.sin(Math.toRadians(latitudeTop)) 
                                - Math.sin(Math.toRadians(latitudeBot))) / 2;
